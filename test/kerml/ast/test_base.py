...
class TestEConn:
    ...
    def test_is_subset(self, env):
        e1 = MockElement(env)
        e2 = MockElement(env)

        c1 = EConn(env, type_=MockElement)
        c2 = EConn(env, type_=MockElement)
        ...
        assert not c1.is_subset([])
        assert c1.is_subset([e1])
        assert not c1.is_subset([e2])
        assert c1.is_subset([e1, e2])
        assert not c2.is_subset([])
        assert not c2.is_subset([e1])
        assert c2.is_subset([e2])
        assert c2.is_subset([e1, e2])
    ...
    def test_is_superset(self, env):
        e1 = MockElement(env)
        e2 = MockElement(env)

        c1 = EConn(env, type_=MockElement)
        c2 = EConn(env, type_=MockElement)
        assert c1.is_superset(c2)
        assert c2.is_superset(c1)

        c1.update([e1])
        c2.update([e2])
        assert not c1.is_superset(c2)
        assert not c2.is_superset(c1)
        assert c1.is_superset([])
        assert c1.is_superset([e1])
        assert not c1.is_superset([e2])
        assert not c1.is_superset([e1, e2])
        assert c2.is_superset([])
        assert not c2.is_superset([e1])
        assert c2.is_superset([e2])
        assert not c2.is_superset([e1, e2])
        ...
        assert not c1.is_superset([e1, e2])
        assert c2.is_superset([])
        assert c2.is_superset([e1])
        assert c2.is_superset([e2])
        assert c2.is_superset([e1, e2])
        ...
        assert c1.is_superset([])
        assert c1.is_superset([e1])
        assert c1.is_superset([e2])
        assert c1.is_superset([e1, e2])
        assert c2.is_superset([])
        assert c2.is_superset([e1])
        assert c2.is_superset([e2])
        assert c2.is_superset([e1, e2])
